...
func (a *Api) EchoHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.Logger.Error("reading the request body failed", zap.Error(err))
		a.ErrorResponse(w, r, span, "invalid request body", http.StatusBadRequest)
		return
	}
	defer r.Body.Close()
	...
	if len(a.Config.BackendURL) > 0 {
		result := make([]string, len(a.Config.BackendURL))
		var wg sync.WaitGroup
		wg.Add(len(a.Config.BackendURL))
		for i, b := range a.Config.BackendURL {
			go func(index int, backend string) {
				defer wg.Done()

				ctx = httptrace.WithClientTrace(ctx, otelhttptrace.NewClientTrace(ctx))
				ctx, cancel := context.WithTimeout(ctx, a.Config.HttpClientTimeout)
				defer cancel()

				backendReq, err := http.NewRequestWithContext(ctx, "POST", backend, bytes.NewReader(body))
				if err != nil {
					a.Logger.Error("backend call failed", zap.Error(err), zap.String("url", backend))
					return
				}
				...
				if resp.StatusCode >= 400 {
					a.Logger.Error("backend call failed", zap.Int("status", resp.StatusCode), zap.String("url", backend))
