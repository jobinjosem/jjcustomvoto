...
func main() {
	...
	// sd.Graceful(stopCh, httpServer, httpServer, grpcServer, healthy, ready)
}

func openGrpcClientConnection(host string) *go_grpc.ClientConn {
	log.Printf("Connecting to [%s]", host)
	conn, err := go_grpc.Dial(
		host,
		go_grpc.WithInsecure(),
		go_grpc.WithStatsHandler(new(ocgrpc.ClientHandler)))
