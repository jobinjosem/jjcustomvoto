...
func (a *Api) TokenValidateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if authorizationHeader == "" {
		a.ErrorResponse(w, r, span, "authorization bearer header required", http.StatusUnauthorized)
		return
	}
	...
	if err != nil {
		a.ErrorResponse(w, r, span, err.Error(), http.StatusUnauthorized)
		return
	}

	if token.Valid {
		if claims.StandardClaims.Issuer != "podinfo" {
			a.ErrorResponse(w, r, span, "invalid issuer", http.StatusUnauthorized)
		} else {
		...
		a.ErrorResponse(w, r, span, "Invalid authorization token", http.StatusUnauthorized)
	}
}
